...
DOCS = [
 ...
 dict(slug="specification-for-the-insurance-ecosystem-pack",
  title="Brief For The pki.sgit.ai Site Agent: Specification For The Insurance Ecosystem Pack, What To Inventory Before Writing It, And The Four Findings That Are Already Established",
  md="v0.33.62__dev-brief__brief-for-the-pki-site-agent-specification-for-the-insurance-ecosystem-pack.md",
  version="v0.33.62", date="26 August 2026 (received 3 September)", dtype="Dev brief \u2014 the fourth of 26 August, handed over rather than read there",
  summary="The specification for a pack this brief deliberately does not contain, because writing it needs the codebase, the vault applications and the tooling surface. The objective: an end-to-end ecosystem on vaults where a new session is told the rules, handed its own policy, measured while it works, and refused by something that is not itself when it exceeds cover, visible in a room. First the inventory \u2014 nine items, none available to the session that wrote this \u2014 then the four findings to inherit rather than re-derive: the ledger must be an append lane because a write key grants purge; hook types include an HTTP variant, which is the difference between a setting and a boundary; token usage has four counters and the obvious one measured 562 for a session that moved a hundred and nine million; and June and August use mandate for opposite things, with August recommended. Seven sections the pack must cover, eleven questions it must answer (the ninth a decision), a first increment that produces a real refusal before any vault exists, and one acceptance test for the pack itself: a session that has read only the pack builds it all and asks nobody a question.",
  concepts=[
   ("The pack it specifies", "../packs/insurance-ecosystem/index.html", "written 3 September, after the inventory"),
   ("The inventory, done", "../packs/insurance-ecosystem/eleven-answers.html", "the nine items with evidence, and the eleven answers"),
   ("The first increment", "../packs/insurance-ecosystem/first-increment.html", "three git policies to two hooks, run"),
   ("The append lane", "../shipped/index.html", "why the ledger cannot be a shared vault"),
   ("Question nine", "../packs/insurance-ecosystem/concepts.html", "mandate is the narrow thing; August governs"),
  ],
  ideas=[
   "The pack cannot be written honestly until somebody establishes what already exists.",
   ...
   "The word mandate names the maximum in one part of the corpus and the minimum in another, which is the most expensive kind of collision because both readings are plausible.",
   "Refusal must come from outside the agent's reasoning loop; balance derivation never runs in the agent; silence below cover is a requirement.",
   "A question an implementing session has to ask is a gap in the pack rather than in the reader.",
  ],
  pages="Answered by packs/insurance-ecosystem: eleven documents plus change control, tools/, hooks/, policies/, a ledger and a room; build-order step 1 built and its acceptance tests run on 3 September."),

 dict(slug="insurance-is-junes-underwriting",
  title="Insurance Is June\u2019s Underwriting Pillar With A Unit Of Payment: The Premium Is Paid In Allocation From A Finite Pool, Pooling Is The Mechanism And Correlation Is What Breaks It, And Recoverability Decides What May Be Insured At All",
  md="v0.33.62__arch-brief__insurance-is-junes-underwriting-with-a-unit-of-payment-premium-paid-in-allocation-correlation-breaks-the-pool.md",
  version="v0.33.62", date="26 August 2026 (received 3 September)", dtype="Architecture brief \u2014 the third of 26 August; the economics the pack implements",
  summary="The insurance model placed against the corpus position it continues: an 18 June brief already established that accepting a risk is underwriting it, that the risk exists the moment the permission is provisioned, that people engage only when they click accept, and that acceptance flows upward with each party underwriting. What this adds is the unit of payment and a claim that settles without a person. The premium is paid in allocation from a pool that is finite by decision at the top, which makes the scarcity the feature. Pooling is the mechanism and correlation is what breaks it, because agent draws are unusually correlated through shared models, templates and dependencies, so the pool needs a reserve the automatic mechanism cannot reach and the correlation must be measured from the first week. Moral hazard is present and experience rating is the control, landing on the team because the agent is indifferent to the loss. A draw is an acceptance event whose acceptor is the policyholder. Three risk zones, and outside cover is uninsured rather than over-insured. Recoverability decides insurability, so bytes into history get an exclusion. A policy may only be written in units the system already counts, the ledger is generic on unit type, showback and chargeback cannot refuse and allocation can, the rate table needs an owner, and a vulnerability is a repricing event with three responses.",
  concepts=[
   ("The policy object", "../packs/insurance-ecosystem/policy-object.html", "the economics as fields: allocation, reserve, draw mode, exclusion with reason"),
   ("Three zones", "../packs/insurance-ecosystem/concepts.html", "below, drawing, outside \u2014 and outside escalates"),
   ("The resource pool", "../insurance/the-resource-pool.html", "doctrine 11, which this brief extends with correlation and the reserve"),
   ("The claim is the draw", "../insurance/the-claim-is-the-draw.html", "doctrine 12: settled in milliseconds because the event is the meter reading"),
   ("Correlation", "../packs/insurance-ecosystem/interface.html", "the room's fourth card, specified from week one"),
  ],
  ideas=[
  ...
  pages="Implemented, not restated, by packs/insurance-ecosystem: the policy object carries the allocation, the reserve, the draw mode and the exclusions; the room carries draw frequency and correlation; the exhaustion workflow escalates."),

 dict(slug="the-enforcement-ladder",
  title="The Enforcement Ladder: Six Levels, A Measured Assurance, And A Catch Above The Hook Is An Incident",
  md="v0.33.85__strategy-brief__the-enforcement-ladder-six-levels-a-measured-assurance-and-a-catch-above-the-hook-is-an-incident.md",
  version="v0.33.85", date="3 September 2026", dtype="Strategy brief \u2014 memo 13, recorded before the project lead had read the pack it describes",
  summary="One policy, the number and size of files committed, walked up every place it could be enforced: nothing; a line in a prompt; a skill or a system prompt; a git hook in the agent's own clone; a check at the destination; and out-of-band verification after the fact. The reading places the six levels on the corpus's three tiers and finds a fourth the tier test had not named: detection. The memo's sharpest claims: assurance per level is measured on ordinary usage rather than asserted; the first four levels are client side, limited and a great signal in the same breath, because a breach seen at the destination means something is seriously wrong, a different policy, a different buffer, and possibly the loss of the licence to operate. Then the practical half: start with a published skill, a script and examples; look for a standard; find a name; and tie it to the grant and the mandate with a default-deny list. Built the same day: a revised policy naming the levels, a level on every ledger event, a reconciliation job that replays git against the ledger, a destination check in report mode, and a room card.",
  concepts=[
   ("Doctrine 13", "../insurance/the-enforcement-ladder.html", "the reading, as doctrine"),
   ("The three-tier control test", "../packs/grant-and-mandate/concepts.html", "what the six levels refine"),
   ("Decision points", "../packs/insurance-ecosystem/decision-points.html", "the levels mapped to hooks, CI and the reconciliation job"),
   ("The room", "../packs/insurance-ecosystem/room/index.html", "catches by level, the sixth card"),
   ("Change control", "../packs/grant-and-mandate/change-control.html", "GM-D104 to GM-D108"),
  ],
  ideas=[
  ...
  pages="Adds insurance doctrine 13 and, in packs/insurance-ecosystem: policy revision r2 with the levels and files_per_commit, a level on every event, tools/reconcile.py, a policy-report CI workflow in report mode, the room's sixth card, and amendments to documents 01 to 08 with IE-D17 onward."),

 dict(slug="the-readings-audited",
  title="The Eleven Readings, Audited Against Their Transcripts",
  md="v0.33.82__audit-brief__the-eleven-readings-audited-against-their-transcripts.md",
  version="v0.33.82", date="31 August 2026", dtype="Audit brief — no memo behind it",
  summary="The site agent auditing the site agent, at the project lead's request: every one of the eleven insurance readings re-read against the transcript it came from, and the four load-bearing corpus quotations re-verified out of the pre-pivot files they cite. The verdict is that the method held and the arithmetic did not. What was got right was the expensive stuff — the two-stage split, the three different things one word was covering when memo 5 said 'not the broker' about an insurance broker and memo 6 meant execution brokers, the Fibonacci repair that saves a settled decision from an instinct that appeared to contradict it, and the 2D-first sequencing labelled honestly as a disagreement with the memo. All four quotations verify verbatim at the dates claimed. Six defects were found and fixed. Three were stale counts, which is the third appearance of that class in this folder, so the fix is a build gate rather than three edits — and the gate caught real text on its first run. One was a category error: the enforcement tier was identified with the incident-response team's quantity, and insurance splits a loss into how often and how bad, of which the tier is neither. One was an over-claim of structural immunity to moral hazard that doctrine 09 contradicts without noticing, because a level is a badge and a badge substitutes for control with no carrier in sight. One was a question dropped between brief and doctrine — memo 3 asked whether a model vendor would carry a policy against its own mistakes, three memos before the same argument was made about brokers and presented as new.",
  concepts=[
   ("The insurance folder", "../insurance/index.html", "the eleven documents audited"),
   ("Why insurance, and what broke it", "../insurance/why-insurance-and-what-broke-it.html", "where the tier was identified with the wrong quantity"),
   ("The ecosystem and the gate", "../insurance/the-ecosystem-and-the-gate.html", "the immunity over-claim, and the rider read at last"),
   ("Who pays, and the rating that moves", "../insurance/who-pays-and-the-moving-rating.html", "the dropped question, restored"),
   ("Change control", "../packs/grant-and-mandate/change-control.html", "GM-D81 to GM-D85"),
  ],
  ideas=[
   "The method held. The arithmetic and the housekeeping did not.",
   "A number typed into prose, in a folder whose own rule is that a claim must be checkable — the third time this class of defect has appeared here, so the fix is a gate, not three edits.",
   "The gate caught real text on its first run: the correction note that recorded a false claim by restating it. The note was reworded rather than the gate loosened.",
   "Insurance splits a loss into how often and how bad. An incident response team is the second. The enforcement tier is neither — it changes how many attempts become losses at all.",
   "The tier ranks reachability. That is blast-radius reduction, not severity reduction, and blast radius is a proxy that is weakest exactly where a small reach touches something critical.",
   "Removing the payout removes one channel of moral hazard and opens another: a level is a badge, and a badge substitutes for control without a carrier anywhere near it.",
   "Any party claiming to reduce somebody else's exposure should carry cover against being wrong about it — broker, platform, or model vendor.",
   "A correction only the correcting document knows about is not a correction.",
   "An endorsement is an append. The estate held the endorsement mechanism before it had a policy to attach one to.",
   "Every defect found here is internal. The one thing that could establish whether any of this is right about insurance is still the survey, and it still has not been run.",
  ],
  pages="Audits all eleven insurance readings against their transcripts and fixes six defects in place. Adds a build gate in gen_insurance.py against stale series arithmetic. Proposes GM-D81 (a printed count is derived, never typed), GM-D82 (the tier ranks reachability, narrowing GM-D52), GM-D83 (any party claiming to reduce exposure carries cover against being wrong), GM-D84 (removing the payout opens a second moral-hazard channel, correcting doctrine 02) and GM-D85 (an endorsement is an append)."),
 ...
 dict(slug="probes-not-tables",
  title="The Grant And Mandate Repo Ships Probes Rather Than Tables: A Grant Claim Travels With The Command That Establishes It",
  md="v0.33.64__dev-brief__the-grant-mandate-repo-ships-probes-not-tables-grants-are-measured-per-tool-and-contributed-mandates-are-elicited-locally-and-never-leave.md",
  version="v0.33.64", date="4 September 2026", dtype="Dev brief — fifth of 4 September; the seed profile in it was probed rather than described",
  summary="A specification for a public registry of capability primitives and measured grants, whose one design decision is that a grant claim never travels alone: it travels with the probe that established it, so a challenge is a rerun with a different result rather than two people asserting different tables. The primitive is a verb crossed with an object class crossed with a reach, held at a stated altitude and carrying whether its effect is reversible; a specific path is an instance, never a new primitive. The unit of mapping is a tool, not a product, because two tools in one session reached different hosts. Grants are public and contributed; mandates are elicited locally through questions about work and never leave the clone. An incident is evidence that demotes a control's claimed evidence mode. A code host rather than a vault, because vaults publish read keys and never write keys. Built here as /probes/: the registry, the finding shape, the runner, seven profiles (two measured), three evidence files.",
  concepts=[
   ("Probes, not tables", "../probes/index.html", "the registry, built"),
   ("The capability primitive", "../probes/index.html#primitive", "verb × object × reach + reversible"),
   ("Grants public, mandates private", "../probes/index.html#split", "the adoption argument, on the page"),
   ("Incidents", "../probes/index.html#incidents", "evidence that demotes"),
   ("The precedents", "the-precedents.html", "which corrected this brief's vocabulary the same day"),
  ],
  ideas=[
   "The unit of contribution is a probe, so a disagreement resolves or it localises.",
   "Reversibility sits on the primitive, because it decides whether a gap is a nuisance or a loss.",
   "A profile is a named configuration listing its tools; web, local and confirmations-disabled are siblings whose diff is the answer.",
   "Nothing about your estate leaves your clone: the delta is computed locally from a public grant and a private mandate.",
   "An incident demotes a control, which is how the registry gets more honest over time rather than more confident.",
   "Value must arrive at the moment a profile is named, before the user has written anything.",
   "Call them capability primitives wherever meters could be meant.",
  ],
  pages="Adds /probes/ (primitives.json, probes.json, run.py, three schemas, seven profiles, three evidence files, incidents, reductions, the questionnaire template, yours/ and submissions/), admin/build/gen_probes.py with its gates, and the bench entry."),

 dict(slug="name-the-question",
  title="Name The Question Rather Than The Concept: A Surprise Is An Action Outside The Grant, So The Assessment Is Falsifiable, And The Scan Cannot Run From Outside",
  md="v0.33.64__strategy-brief__name-the-question-not-the-concept-a-surprise-is-an-action-outside-the-grant-so-the-assessment-is-falsifiable-and-the-scan-cannot-run-from-outside.md",
  version="v0.33.64", date="4 September 2026", dtype="Strategy brief — sixth of 4 September; naming candidates tested against the corpus, availability not checked",
  summary="The memo asks for a name for the gap, a public site that lets anybody measure their own, and the commercial model. Tested against the corpus, every clean naming candidate is obscure and every familiar one collides, so the recommendation is not to coin a noun: the technical term stays excess authority and the public phrase is a sentence, what you authorised and never asked for. Two ideas do real work. A surprise is an action outside the grant and never a wrong action inside it, which makes the assessment falsifiable: zero surprises over a period means the measurement held. And the precedent services could scan you from outside, while agent exposure cannot be seen from outside, so the site hosts the questions and the interpretation and the measurement runs in the user's environment. The assessment expires when a vendor moves a default; the commercial model is free through level three and paid where somebody's own environment is modelled. Built here as /authorised/.",
  concepts=[
   ("What you authorised and never asked for", "../authorised/index.html", "the page, built"),
   ("The surprise definition", "../authorised/index.html#surprise-def", "the falsifiable validity test"),
   ("The assessment expires", "../authorised/index.html#expires", "three sources of movement"),
   ("The ladder", "../authorised/index.html#ladder", "free through level three"),
   ("Excess authority", "../packs/registry-mvp/index.html", "the settled technical term"),
  ],
  ideas=[
   "Unauthorised is wrong on the first word: the grant is exactly what was authorised.",
   "A coined noun asks the reader to learn something before they can care.",
   "An action inside the grant is never a surprise, however wrong it was.",
   "The site cannot scan anybody, and that is the privacy story: it cannot see you even if it wanted to.",
   "Step three is the test of the whole thing: name three products and see a grant you did not know you had.",
   "The site collects nothing by default and offers a submission of counts only.",
  ],
  pages="Adds /authorised/ (index.html, app.js, README.md with the precedents record), assets/authorised.css, and the bench entry."),

 dict(slug="the-precedents",
  title="The Precedents Lead With A Grade, Scale By Selling The Same Thing Larger, Ship A Documented JSON Output, And The Probe And Finding Vocabulary Already Exists",
  md="v0.33.64__research-brief__the-precedents-lead-with-a-grade-and-scale-by-selling-the-same-thing-larger-and-the-probe-and-finding-vocabulary-already-exists.md",
  version="v0.33.64", date="4 September 2026", dtype="Research brief — seventh of 4 September; six services read from their own published pages, none operated end to end",
  summary="Six free assessment services examined on the axes the memo asked about. Their shared shape: one small input, a single legible verdict above the fold with the evidence beneath it, no account for the basic case, and a result the visitor wants to show somebody. The ladder that reaches organisations sells the same thing larger rather than something different. The services that reach engineering publish a documented machine-readable output and run where the thing lives. The finding to act on immediately: the probe and finding vocabulary proposed the same day already exists in OpenSSF Scorecard with definitions close enough to adopt, and that project replaced a score with structured findings for the reason this corpus gave against a levels ladder. Independence is the thing worth charging for. Acted on here: the finding shape adopts probe, message, outcome, remediation and location, and adds reversibility and tier.",
  concepts=[
   ("The finding shape", "../probes/schema/finding.schema.json", "Scorecard's, plus reversibility and tier"),
   ("The verdict as a statement", "../authorised/index.html", "a count, not a grade"),
   ("Independence is what costs", "../authorised/index.html#ladder", "the evidence tier as a price list"),
   ("The record", "../authorised/README.md", "what transferred and what did not"),
  ],
  ideas=[
  ...
  pages="Shapes /probes/schema/ and the verdict on /authorised/; the record is authorised/README.md."),

 dict(slug="guess-the-agent",
  title="Guess The Agent: The Tree Asks Cheap Questions, Guesses Your Grant, And The Product Is The Gap Between What You Predicted And What It Found",
  md="v0.33.64__dev-brief__guess-the-agent-the-tree-guesses-your-grant-and-the-product-is-the-gap-between-what-you-predicted-and-what-it-found.md",
  ...
  concepts=[
   ...
   ("The profiles", "../probes/index.html#profiles", "nothing to guess without them"),
  ],
  ideas=[
