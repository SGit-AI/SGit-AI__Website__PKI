...
""",
 ...
 dict(slug="grant-and-mandate",
  ...
  docs=[
   ...
   dict(slug="change-control", file="99__change-control.md",
    title="Appendix — Change control",
    role="Ten corrections, twenty-two decisions, and the refusal the pack found by accident",
    summary="The appendix records what the two source briefs settle (GM1–GM8: the grant discovered not authored, authority not authorisation, the library/instance split, reality before risks, the three-term comparison, memory as a time axis, adopt-Cedar-and-graphs-build-only-the-mandate, and the naming checks) and what the pack's own construction added (GM9–GM10: the first library entry refusing to measure itself, and node n3 as the thesis caught live). The decisions register carries twenty-two entries, fourteen settled by the briefs and eight open — who measures a library entry and with what authority, how often it is re-measured, whether the instance records which library version it referenced, the history-window field, where a built pack lives, whether the blind-spot number is published per agent, whether a hook can trust the mandate it reads, and the corpus version on adoption.",
    concepts=[
