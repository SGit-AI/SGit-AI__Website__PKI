...
**Status:** a design pack with **one shipped consumer**. Fourteen documents plus three appendixes — a PR/FAQ, a PEP-style specification and change control; the registry itself is unbuilt, and the assessment at `/assess` is built and live · site-agent authored · awaiting project-lead adoption (corpus version assigned then)
**Date:** 20 August 2026
...
17. `99__change-control.md` — **Appendix C.** Every correction and every decision. Read it *second* if you are about to build from 00–04, so you read them with the errata in hand; read it *last* if you are reading the pack through. Never not at all
