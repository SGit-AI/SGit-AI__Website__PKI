...
kept going every time, because one click had already produced 13 / 9 / 0 and a named weakest link.
R1 was predicted to bounce off the jargon before reaching the finding; a result arrived first.
...
lecture."* The framing most likely to be rejected was accepted specifically.

**The picture behaved as document 07 predicted.** That document warns the visual washes out framing
effects and says test visual variants first; the run's only unqualified moment — *"the picture is the
moment it clicked"* — came from a rendering, not a sentence.
...
so the five hidden rows are by construction the *least* alarming — on solo-dev, "read your shell
history" and "draft or edit a document for me". The reader said *"I want to know what else is hiding
in there"*: the truncation swapped five mild facts for an imagined worse set. Document 02's
benign-nodes rule exists because *"a tree that only lists frightening capabilities is measuring its
own framing rather than the visitor's setup"* — and `slice(0, 4)` over a weight-sorted list re-imposes
that framing at the one place a reader within budget looks. A P6 problem wearing a UI bug's clothes.
...
passes, because it checks the `why` is *available* on click, not that anybody finds it: document 12's
lesson 3 at the story layer.
...
intention, not an act, and document 08 puts "does it feel compelling enough to act on" out of scope.
Record it as *the card reads as sendable*, no further.

Also not evidenced: **the controls half**, never reached. P8 requires every path to end on something
the visitor can do, and its check enumerates end-states of the *document*. The check is scoped
wrongly: the end-state that matters is where the budget ends. P8 survived here only because the
sentence at the top happened to be an action.
...
6. **The vocabulary — keep MC-D23 open.** *(structural.)* The reader called the eyebrow *"jargon
   ...
   agents can reach, and what you meant"* and the technical eyebrow sits above it. Lead with the h1
   and let the terms appear where the computation defines them — the arrival cost paid without
   spending the vocabulary. Note too that F2 conflates two owners: the eyebrow is the assessment's
   copy, the nav is site chrome, and a tool arriving by shared link carries the whole site's
   furniture — a document 09 question, not a document 06 one.
...
pixel boundary, a local artefact, n = 1. Add three. Nothing here supports a claim about **tone** —
*"respected my time"* is a preference, which document 08 puts out of scope. Nothing supports the
**sharing** claim, which was an intention. Nothing supports any claim about **the controls
section**, whose absence from the transcript is a fact about the budget.
...
**Un-truncate the delta.** Not because it is the largest defect — the vocabulary question is more
consequential — but because it is the only one that both cost budget *and* inverted a principle. The
rest is friction. This made the page quietly more alarming than its own data, at the moment the
reader was deciding whether to trust it, in a tool whose sixth principle is that nothing on the page
may be optimised for how alarming it feels. It is also the cheapest item on the list. Do it before
run 008.
