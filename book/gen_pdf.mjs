...
next release.</p>
<p>The six terminal figures are printed in the chapters as their real transcript text, which is what
makes this PDF readable offline; the photographed capture of each is recorded in
<code>book/shots/shots.json</code> with its digest. Appendix A is the whole harness.</p>
...
try {
  ...
  await page.pdf({
    path: `${BOOK}/a-key-means-nothing-alone.pdf`,
    format: 'A4',
    ...
    footerTemplate:
      ...
      '<span>A Key Means Nothing Alone &middot; pki.sgit.ai &middot; CC BY 4.0</span>' +
...
console.log('  a-key-means-nothing-alone.pdf written');
